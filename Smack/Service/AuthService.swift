    ...
    func registerUser(email: String, password: String, completion: @escaping CompletionHandle) {
        ...
        Alamofire.request(URL_REGISTER, method: .post, parameters: body, encoding: JSONEncoding.default, headers: HEADER).responseString{
            ...
            if response.result.error == nil {
                ...
                completion(false)
                debugPrint("RegisterUser Error -> \(response.result.error as Any)")
            }
        }
    }
...
//                }
                ...
                completion(false)
                debugPrint("LoginUser Error -> \(response.result.error as Any)")
            }
        }
    }
    ...
    func createUser(name: String, email: String, avatarName: String, avatarColor: String, completion: @escaping CompletionHandle) {
        ...
        Alamofire.request(URL_USER_ADD, method: .post, parameters: body, encoding: JSONEncoding.default, headers: BEARER_HEADER).responseJSON { (response) in
            ...
            if response.result.error == nil {
                ...
                completion(false)
                debugPrint("CreateUser Error -> \(response.result.error as Any)")
            }
        }
    }
